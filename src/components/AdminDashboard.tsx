  ...
  return (
    <section className="grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
      <div className="rounded-[30px] border border-black/5 bg-white p-6 shadow-[0_24px_60px_-42px_rgba(15,23,42,0.35)]">
        ...
        <h2 className="mt-2 text-2xl font-semibold text-stone-950">Criar ou editar sorteio</h2>
      ...
      <div className="space-y-4">
        <div className="rounded-[30px] border border-black/5 bg-white p-6 shadow-[0_24px_60px_-42px_rgba(15,23,42,0.35)]">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div>
              <p className="text-sm font-semibold uppercase tracking-[0.22em] text-stone-500">Sorteio ativo</p>
              <h2 className="mt-2 text-2xl font-semibold text-stone-950">{activeRaffle?.title ?? 'Nenhum sorteio'}</h2>
              {activeRaffle && <p className="mt-1 text-sm text-stone-600">Status: {statusLabel[activeRaffle.status]}</p>}
            </div>
            <span className="rounded-full bg-stone-100 px-3 py-1 text-xs font-semibold text-stone-600">{raffles.length} sorteio(s)</span>
          </div>
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            <button type="button" onClick={onOpen} className="rounded-full bg-brand-600 px-4 py-2 text-sm font-semibold text-white">Abrir</button>
            <button type="button" onClick={onClose} className="rounded-full bg-stone-950 px-4 py-2 text-sm font-semibold text-white">Fechar</button>
            <button type="button" onClick={onDraw} className="rounded-full bg-amber-500 px-4 py-2 text-sm font-semibold text-white">Sortear auto</button>
            <button type="button" onClick={onComplete} className="rounded-full border border-black/10 bg-white px-4 py-2 text-sm font-semibold text-stone-700">Concluir</button>
            <button type="button" onClick={onDelete} className="rounded-full border border-red-200 bg-red-50 px-4 py-2 text-sm font-semibold text-red-700">Excluir</button>
          </div>
        </div>
